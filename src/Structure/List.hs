...
maap f = foldRight (\x a -> f x :. a) Nil
...
fiilter f = foldRight (\x a -> if f x then x :. a else a) Nil
...
seqOptional = foldRight (\ho acc ->
  bindOptional ho (\h ->
    mapOptional (h :.)
  acc)) (Full Nil)


--seqOptional = error "err"
